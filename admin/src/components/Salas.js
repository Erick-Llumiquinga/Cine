...
export default class Salas extends Component {
    ...
    editarSala(sala){
      ...
      let data = {
        _id: sala._id,
        nombre: this.state.nombre == "" ? sala.nombre : this.state.nombre,
        descripcion: this.state.descripcion == "" ? sala.descripcion : this.state.descripcion,
        horario: this.state.horario == "" ? sala.horario : this.state.horario,
        idPelicula: this.state.idPelicula == "" ? sala.idPelicula : this.state.idPelicula,
      }
                      ...
                      <tr key={sala._id} >
                        ...
                        <td class="py-4 px-10 bg-grey-lightest font-bold uppercase text-sm ">
                          {
                            this.state.peliculas.map(item => {
                              if(sala.idPelicula == item.id){
                                return(
                                  <select name="idPelicula" onChange={this.handleChange}>
                                    <option value="999" >{item.titulo}</option>
                                    {
                        ...
                        <div class="flex" >
                          <div class="flex-1 text-green-700  px-2 py-6">
                            <button class="uppercase bg-grey-lightest font-bold uppercase text-sm" onClick={() => this.editarSala(sala)} >
